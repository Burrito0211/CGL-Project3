...
//==========================================================================
{
	switch(e) {
		...
		case FL_MOUSEWHEEL: {
			...
			eyeZ *= zamt;
			// Prevent zooming too close to the near clip plane which can
			// make lights or geometry disappear; clamp to a reasonable min.
			const float kMinEyeZ = 1.0f;
			if (eyeZ < kMinEyeZ) eyeZ = kMinEyeZ;
			wind->damage(1);
			return 1;
			};
			break;
